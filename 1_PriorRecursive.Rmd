...
```{r}
metropolis <- function(X, y, M, KDE = FALSE, C = NA, den_funs = NA, N = 20000, a_target = 0.234, b1 = rep(0, M), batch = TRUE, batch_schedule = seq(50, N, by = 50)) {
  #0) Initial values
  if (batch == FALSE) {
...
```{r}
prior_recursive <- function(k, X, y, M, N = 20000, a_target = 0.234, b1 = rep(0, M)) {
  #Partition the data
  #Number of rows
  n <- nrow(X)
  ...
  for (j in seq(1,k)) { 
    if (j == 1) {
      C = metropolis(parts_X[[j]], parts_y[[j]], M, KDE = FALSE, C = NA, den_funs = NA, N = N, a_target = a_target, b1 = b1, batch = TRUE)
      ...
      C = metropolis(parts_X[[j]], parts_y[[j]], M, KDE = TRUE, C = C, den_funs = den_funs, N = N, a_target = a_target, b1 = modes, batch = TRUE)
