...
Two-stage MCMC first partitions the data into subsets, runs a local MCMC on each partition to estimate their parameters, and then in the second stage summarizes the posterior estimates of the parameters to output aggregated distributions. While running a single MCMC on an entire, voluminous dataset would be most optimal, it may be computationally infeasible due to finite memory resources. The two-stage method breaks the dataset into manageable, parallizable sets. Hepler et al. demonstrate that this recursive approach is computationally more efficient and eliminates the need for approximations or dimension reduction.
...
The spatio-temporal data, USDMDataAvg.csv, can be found [here](https://datadryad.org/stash/dataset/doi:10.5061/dryad.g1jwstqw7). Erhardt et al. created this dataset from the original US Drought Monitor (USDM) from continuous shape files to a discretized spatial support defined by grid cells of size 0.5 degrees latitude by 0.5 degrees longitude [1]. The response variable is an ordinal measure of the severity of drought in the United States. The USDM levels are:
...
To ease the computational demands of this demonstration, in this document we only focus on drought levels in Arizona since the beginning of 2020, reflected in the subset datafile USDMData_AZ.csv. The .R file that created this subset data along with the .csv data itself can be found [here](https://github.com/SashaCroatia/BayesianInference/tree/main/IRP).
...
Below is an example of what the ordinal drought levels are for Arizona at $t=1$, corresponding to the first week of January, 2020:
...
$$
Y_{i,t}\in\{0,1,2,3,4,5\} = \{0, D0, D1, D2, D3, D4\}
$$ so $\alpha = (-\infty,0,1,2,3,4,\infty)$. The latent (unobserved) continuous drought process is

$$
Z_{i,t} = \begin{cases}X_{i,t}{\beta}_i + \epsilon_{i,t}, & \text{for } t = 1 \\[6pt]X_{i,t}{\beta}_i + \rho_i \big(Z_{i,t-1} - X_{i,t-1}{\beta}_i\big) + \epsilon_{i,t}, & \text{for } t > 1,\end{cases}
$$

where $X_{i,t}$ is a $(P+1)$-dimensional vector of covariates including the intercept parameter, $\beta_{i} = (\beta_{0i},\ldots,\beta_{Pi})$ is the location specific $(P+1)$-dimensional spatial random effect vector, $\rho_i$ is a spatial dependent autoregressive term, and errors $\epsilon_{i,t}\sim N(0,\sigma_i^2)$. In our example, $P=3$.

Let $\gamma_i = logit(\rho_i)$, $\theta_{Z,i} = (\beta_i,\gamma_i,\sigma_i^2)$ , and $\phi = (\sigma_\gamma^2,\{\sigma_p^2,p = 0,\ldots,P\})$.

In their paper, Hepler et al. assume $\beta_i$ and $\gamma_i$ independently follow intrinsic conditional autoregressive (ICAR) spatial models. The ICAR model is similar to the CAR model whereby they both analyze spatially correlated local dependency between random variables in areal (lattice) data. For these ICAR priors, we must calculate the adjacency matrix (Queen's adjacency) based on the distance between each spatial coordinate.

Lastly, all variance parameters $\sigma_i^2, \sigma_\gamma^2, \text{and } \sigma_p^2$ follow inverse gamma prior distributions.

Putting this together, the full Bayesian hierarchical model of the spatio-temporal is:

$$
\begin{aligned}\text{Data Model:} \quad & Y_{1:T} \mid Z_{1:T} \sim f(Y_{1:T} \mid Z_{1:T}), \\[6pt]\text{Process Model:} \quad & Z_{1:T} \mid {\theta}_Z, X_{1:T} \sim \pi(Z_{1:T} \mid {\theta}_Z, X_{1:T}), \\[6pt]\text{Prior Models:} \quad & {\theta}_Z, {\phi} \sim \pi({\theta}_Z \mid {\phi}) \pi({\phi}).\end{aligned}
$$

The posterior distribution is:

$$
\pi(Z_{1:T}, {\theta}_Z, {\phi} \mid Y_{1:T}, X_{1:T})\;\propto\;f(Y_{1:T} \mid Z_{1:T})\,\pi(Z_{1:T} \mid {\theta}_Z, X_{1:T})\,\pi({\theta}_Z \mid {\phi})\,\pi({\phi})
$$
...
**Note:** To save on space and improve readability, for the remaining plots in this section we omit their corresponding code because they're almost identical.
...
The following implements the two-stage MCMC algorithm. In practice, we would've used an HPC for stage one of the algorithm to take full advantage of the parallel nature of this method, whereby we run an MCMC sampler for each parameter at each spatial location. However, as this is only a demonstration, we ran the code sequentially using a for loop on a local machine. Regardless, after stage 1, re-sampling all the individual samples in stage 2 is fast and can be done locally. Find the commented code that implements it in the Appendix.
...
sd_beta2 <- apply(beta.out, c(1, 2), sd) #get sd of the posteriors
...
intercept_sd2 <- sd_beta2[,1]
evp_sd2 <- sd_beta2[,2]
soilm_sd2 <- sd_beta2[,3]
tsoil_sd2 <- sd_beta2[,4]
```
...
ggplot() +
  ...
  geom_sf(data = az_boundary, fill = NA, color = "black", linewidth = 0.7) +
  ggtitle("SD of Intercept - Two Stage") +
  coord_sf(xlim = c(-115, -109), ylim = c(31, 37)) +
  theme_minimal() +
  theme(
    plot.title = element_text(size = 14, face = "bold"),
...
ggplot() +
  ...
  geom_sf(data = az_boundary, fill = NA, color = "black", linewidth = 0.7) +
  ggtitle("SD of Evapotranspiration - Two Stage") +
  coord_sf(xlim = c(-115, -109), ylim = c(31, 37)) +
  theme_minimal() +
  theme(
    plot.title = element_text(size = 14, face = "bold"),
...
ggplot() +
  ...
  geom_sf(data = az_boundary, fill = NA, color = "black", linewidth = 0.7) +
  ggtitle("SD of Soil Moisture - Two Stage") +
  coord_sf(xlim = c(-115, -109), ylim = c(31, 37)) +
  theme_minimal() +
  theme(
    plot.title = element_text(size = 14, face = "bold"),
...
ggplot() +
  ...
  geom_sf(data = az_boundary, fill = NA, color = "black", linewidth = 0.7) +
  ggtitle("SD of Soil Temps - Two Stage") +
  coord_sf(xlim = c(-115, -109), ylim = c(31, 37)) +
  theme_minimal() +
  theme(
    plot.title = element_text(size = 14, face = "bold"),
...
Comparing the effect of each of parameter in $\beta_i$ to the ordinal drought level at each grid location $i$ in Arizona, the single stage and the two stage MCMC algorithms produced visually similar plots. However, we note the following limitations of the two-stage method:

-   the plot of each parameter's standard deviations appear slightly higher overall than in the single stage. This is to be expected given that we're not sampling all-at-once.

-   Hepler et al. mention that since stage 2 uses a regular Metropolis-Hastings update, jointly updating a high-dimensional parameter could greatly lower a sample's acceptance rate. This poor mixing means the MCMC struggles to find other suitable samples in the parameter space.

-   they also mention that the quality of the samples in stage 2 greatly depend on the quality of samples in stage 1. Quality samples means that stage 1 explored the entire parameter space.

Despite these potential drawbacks, in this demonstration we observed only small differences between the samples produced from single and two stage. Assuming our dataset is not too high-dimensional soas to produce poor mixing mentioned above, we recommend the two stage approach because it

-   is computationally efficient due to its potential for parallelization in stage 1.

-   has real-world applications for very large spatio-temporal data sets.

------------------------------------------------------------------------

## References

1.  Erhardt, R., DiVittorio, C. A., Hepler, S. A., Lowman, L. E. L., & Wei, W. (2024). *Homogenized gridded dataset for drought and hydrometeorological modeling for the continental United States*. Scientific Data, 11, Article 375.

2.  Hepler, S., & Erhardt, R. (2025). *Two-stage MCMC for fast Bayesian inference of large spatio-temporal ordinal data, with application to US drought*. arXiv preprint

3.  Lunn, D., Barrett, J., Sweeting, M., and Thompson, S. *Fully Bayesian hierarchical mod- elling in two stages, with application to meta-analysis*. Journal of the Royal Statistical Society. Series C, Applied Statistics, 62(4):551 (2013).

------------------------------------------------------------------------

## Appendix

As we're not seeking the most accurate results in this demonstration, please note the following important modifications to the Hepler et al.'s code to increase computational efficiency. In both the single stage and two stage implementations, rather than slice sampling we use a random walker sampler along with fewer sample iterations.

### Single Stage

Following Hepler et al.'s code [here](https://github.com/heplersa/DroughtModelTwoStageMCMC/blob/main/ComparisonStudy/MCMCstandard.R).
...
Following Hepler et al.'s code [here](https://github.com/heplersa/DroughtModelTwoStageMCMC/blob/main/ComparisonStudy/MCMCstage1.R)
...
Following Hepler et al.'s code [here](https://github.com/heplersa/DroughtModelTwoStageMCMC/blob/main/ComparisonStudy/MCMCstage2.R)
