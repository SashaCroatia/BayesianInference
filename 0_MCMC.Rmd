...
library(mvtnorm)
log_prior <- function(distro, mu, lambda, sigma) {
  # Compute the density at 'distro' under the MVN used in proposal()
  prob <- dmvnorm(distro, mean = mu, sigma = lambda*sigma)
  log_prob = log(prob)
  return(0)
}
```
...
for (i in 2:N) {
    ...
    P_list[i] <- logistic_ll(b_prop, X, y) + log_prior(b_prop, mu, lambda, sigma)  #P(beta*)
